...
export function Header() {
  ...
  return (
    <header
      className={cn(
        'fixed inset-x-0 top-0 z-50 mx-auto w-full max-w-none border-b border-white/15 bg-[#1f1f1f57] shadow-2xl shadow-black/20 backdrop-blur-xl transition-[top,max-width,border-radius,padding,box-shadow,background-color,border-color] duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] will-change-[top,max-width,border-radius] md:rounded-b-[2rem] md:border md:px-6',
        {
          'bg-background/90': open,
          'md:top-4 md:max-w-4xl md:rounded-full md:bg-[#1f1f1f57] md:px-0 md:shadow-2xl md:shadow-black/30':
            scrolled && !open,
        },
      )}
    >
      <nav
        className={cn(
          'mx-auto flex h-16 w-full max-w-6xl items-center justify-between gap-4 px-6 transition-[height,max-width,padding] duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] md:h-16',
          {
            'md:h-14 md:max-w-4xl md:px-3': scrolled,
          },
        )}
      >
        <a href="/" onClick={closeMenu} className="flex min-w-0 items-center gap-3 text-white no-underline" aria-label="DermoScan home">
          ...
          <span className="hidden min-w-0 leading-none sm:block">
            <span className={cn('block font-semibold tracking-normal transition-[font-size] duration-700 ease-[cubic-bezier(0.16,1,0.3,1)]', scrolled ? 'text-sm' : 'text-base')}>DermoScan</span>
            <span className={cn('mt-1 block whitespace-nowrap uppercase text-cyan-100/65 transition-[font-size] duration-700 ease-[cubic-bezier(0.16,1,0.3,1)]', scrolled ? 'text-[9px]' : 'text-[10px]')}>AI lesion review</span>
          </span>
        </a>

        <div className={cn('hidden items-center gap-2 rounded-full border border-white/10 bg-black/10 px-3 py-2 transition-[padding] duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] md:flex', scrolled && 'px-2 py-1.5')}>
          {links.map((link) => (
            <a
              key={link.label}
              className={cn(
                buttonVariants({ variant: 'ghost' }),
                'h-9 rounded-full px-4 text-white/75 no-underline hover:bg-white/10 hover:text-white',
              )}
              href={link.href}
            >
              {link.label}
      ...
      <div
        className={cn(
          'fixed inset-x-0 bottom-0 top-16 z-50 flex flex-col overflow-hidden border-y border-white/10 bg-[#071316]/95 backdrop-blur-2xl md:hidden',
          open ? 'block' : 'hidden',
        )}
      >
        <div
            ...
            {links.map((link) => (
              <a
                ...
                className={buttonVariants({
                  variant: 'ghost',
                  className:
                    'h-auto justify-start rounded-2xl border border-white/10 bg-white/10 px-4 py-4 text-base text-white no-underline hover:bg-white/15 hover:text-white',
                })}
                href={link.href}
              >
                {link.label}
