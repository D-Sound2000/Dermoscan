...
const signalCards: Array<{ label: string; value: string; icon: LucideIcon }> = [
...
const reviewPartners: Array<{ name: string; icon: LucideIcon }> = [
  { name: "Image intake", icon: FileScan },
  { name: "AI triage", icon: ScanLine },
  { name: "Clinical review", icon: Microscope },
    ...
    <>
      ...
      <main className="overflow-hidden bg-background text-foreground">
        <section className="relative isolate min-h-screen overflow-hidden">
          ...
          <div
            aria-hidden
            className="absolute inset-0 -z-10 bg-[linear-gradient(180deg,rgba(7,9,9,.76),var(--background)_78%),radial-gradient(70%_70%_at_50%_20%,rgba(212,175,55,.16),transparent_62%)]"
          />
          <div
            aria-hidden
            className="pointer-events-none absolute inset-x-0 top-24 -z-10 hidden h-[28rem] bg-[linear-gradient(90deg,transparent,rgba(212,175,55,.14),transparent)] opacity-60 blur-3xl lg:block"
          />
          ...
          <div className="mx-auto flex min-h-screen max-w-7xl flex-col justify-center px-6 pb-12 pt-28 md:pt-36">
              ...
              <AnimatedGroup variants={transitionVariants}>
                ...
                >
                  <span className="text-sm text-foreground">
                    AI skin-lesion review workspace
                  </span>
                  <span className="block h-4 w-0.5 border-l bg-white/20" />
                ...
                <p className="mx-auto mt-8 max-w-2xl text-balance text-base leading-8 text-muted-foreground md:text-lg">
                  Upload a dermoscopic or close-up skin photo, receive a benign
                  or malignant probability, and inspect a Grad-CAM heatmap that
                  shows what influenced the model.
                </p>
              </AnimatedGroup>
            ...
            >
              <div className="relative mt-12 overflow-hidden px-0 sm:mt-16 md:mt-20">
                ...
                />
                <div className="relative mx-auto max-w-6xl overflow-hidden rounded-2xl border border-border bg-background/90 p-3 shadow-2xl shadow-black/35 ring-1 ring-background/70 backdrop-blur">
                  <div className="grid overflow-hidden rounded-xl border border-border bg-card md:grid-cols-[1.05fr_.95fr]">
                    <div className="relative min-h-[290px] overflow-hidden bg-muted md:min-h-[390px]">
                      ...
                      <div className="absolute left-8 right-8 top-20 h-px bg-gradient-to-r from-transparent via-primary to-transparent shadow-[0_0_28px_rgba(212,175,55,.75)]" />
                    </div>
                    <div className="flex flex-col justify-between gap-8 p-6 md:p-8">
                      <div>
                        ...
                        <p className="mt-4 text-sm leading-7 text-muted-foreground">
                          DermoScan is designed for transparent review. It pairs
                          probability scores with heatmap context so the result
                      ...
                      <div className="grid gap-3 sm:grid-cols-2">
                        {signalCards.map((signal) => (
                          ...
                          >
                            ...
                            <strong className="mt-4 block text-lg text-foreground">
                              {signal.value}
                            </strong>
        ...
        <section className="bg-background pb-16 pt-12 md:pb-28">
          <div className="group relative m-auto max-w-5xl px-6">
            <div className="absolute inset-0 z-10 flex scale-95 items-center justify-center opacity-0 duration-500 group-hover:scale-100 group-hover:opacity-100">
              <Link href="#evidence" className="block text-sm duration-150 hover:opacity-75">
                <span> Review the evidence layer</span>
                <ChevronRight className="ml-1 inline-block size-3" />
              </Link>
            </div>
            <div className="mx-auto mt-12 grid max-w-2xl grid-cols-2 gap-4 transition-all duration-500 group-hover:opacity-50 group-hover:blur-sm sm:grid-cols-4">
              {reviewPartners.map((partner) => (
                <div
                  key={partner.name}
                  className="flex min-h-24 flex-col items-center justify-center rounded-lg border border-border bg-card/70 p-4 text-center"
                >
                  <partner.icon className="size-5 text-primary" aria-hidden />
      ...
      <nav data-state={menuState && "active"} className="group fixed z-50 w-full px-2">
        <div
          className={cn(
            ...
            isScrolled &&
              "max-w-4xl border-primary/20 bg-background/55 shadow-[0_18px_60px_rgba(0,0,0,.32),inset_0_1px_0_rgba(255,255,255,.18)] lg:px-5",
          )}
        >
          <div
            aria-hidden
